...
const SKILL_CATEGORIES = [
  {
    id: "01",
    key: "professional",
    title: "Chuyên môn CRM & dữ liệu",
    shortTitle: "Chuyên môn",
    score: "86%",
    color: "#007aff", // Blue
    textColor: "text-[#007aff]",
    bgColor: "bg-[#007aff]",
    skills: contentData.skills.professional,
  },
  {
    id: "02",
    key: "leadership",
    ...
    bgColor: "bg-[#ff3b30]",
    skills: contentData.skills.leadership,
  },
  {
    id: "03",
    key: "collaboration",
    title: "Hợp tác & trải nghiệm",
    shortTitle: "Hợp tác",
    score: "89%",
    color: "#af52de", // Purple
    textColor: "text-[#af52de]",
    bgColor: "bg-[#af52de]",
    skills: contentData.skills.collaboration,
  },
  {
    id: "04",
    key: "innovation",
    ...
    bgColor: "bg-[#ff9500]",
    skills: contentData.skills.innovation,
  },
  {
    id: "05",
    key: "languages",
...
// Sparkline Wave data for KPI Cards
...
function SkillsInteractiveRadar({
  ...
  // Calculate polygon vertex points for each concentric level
  ...
  // Calculate static target data points
  const targetDataPoints = data.map((item, i) => {
    const angleDeg = -90 + i * (360 / numAxes);
    const rad = (angleDeg * Math.PI) / 180;

    // Full target coordinates for label alignment and tooltips
  ...
  return (
    <div className="relative flex w-full flex-col items-center justify-center select-none">
      ...
      <div className="relative w-full max-w-[480px]">
        ...
        >
              {/* 2. Concentric Fluent Grid Background Polygons (For Radar & Polar views) */}
          {(chartType === "radar" || chartType === "polar") && levels.map((lvl, index) => {
            ...
            return (
              <g key={lvl} className="pointer-events-none">
                ...
                {/* Concentric circular guide ring for Fluent UI 3 precision radar aesthetics */}
                <circle
                  cx={cx}
                  cy={cy}
                  r={R * (lvl / 100)}
                ...
                {/* Fluent Data Axis Scale Marker Ticks (North Axis) */}
                <g transform={`translate(${cx}, ${cy - R * (lvl / 100)})`}>
                  ...
                  <text
                    ...
                    className="text-[8px] font-extrabold fill-[var(--muted)]"
                  >
                    {lvl}%
                  </text>
                </g>
          ...
          {/* 3. Radial Spoke Axis Lines (For Radar & Polar views) */}
            ...
            return (
              <g key={i}>
                <line
                  ...
                  x2={x2}
                  y2={y2}
                  stroke={isHovered ? item.color : "var(--border)"}
                ...
                <circle
                  ...
                  cy={y2}
                  r={isHovered ? 3.5 : 2}
                  fill={isHovered ? item.color : "var(--border)"}
                  className="transition-all duration-300"
                />
              </g>
            );
          ...
          {/* 4. Animated Filled Radar Polygon (Radar Only) */}
          {chartType === "radar" && (
            <motion.polygon
          ...
          {/* 4b. Dynamic Polar (Coxcomb) Flower-like Petals (Polar Only) */}
          {chartType === "polar" && data.map((item, i) => {
            const angleDeg = -90 + i * (360 / numAxes);
            const halfWidth = 360 / numAxes / 2 - 2; // small spacing gap
            ...
            // Pure SVG path: Move to center, Line to start boundary, Arc to end boundary, close path
          ...
          {/* 5. Interactive Vertex Dots & Pulsing Halo (Radar Only) */}
          {chartType === "radar" && targetDataPoints.map((pt, i) => {
            ...
            return (
              <motion.g
                ...
                style={{ transformOrigin: `${cx}px ${cy}px` }}
              >
                {/* Continuous gentle pulse to indicate interactivity */}
                {!isHovered && (
                  <circle
                    cx={pt.x}
                    cy={pt.y}
                    r={7}
                    fill="var(--app-primary-hex, #7c3aed)"
                    fillOpacity={0.4}
                    className="animate-ping"
                    style={{ animationDuration: '3s', animationDelay: `${i * 0.15}s` }}
                  />
                )}

                {/* Outer pulsing halo on hover */}
                {isHovered && (
                  <>
                    <circle
                      cx={pt.x}
          ...
          {/* 6. Center Overall Score Badge (Fluent UI 3 Circle Card) */}
          <g transform={`translate(${cx}, ${cy})`} className="cursor-default">
            ...
            <text
              ...
              textAnchor="middle"
              dominantBaseline="central"
              className="text-[8px] font-black uppercase tracking-wider fill-[var(--muted)]"
            >
              {isVi ? "ĐIỂM TB" : "OVERALL"}
            </text>
            <text
              y={7}
              textAnchor="middle"
              ...
              style={{ fill: "var(--app-primary-hex, #7c3aed)" }}
            >
              {overallAvg}%
            </text>
          </g>

          {/* 7. Fluent UI 3 Data Axis Labels at Each Vertex (Using Acrylic Pills for Legibility) */}
          {data.map((item, i) => {
            const angleDeg = -90 + i * (360 / numAxes);
            const rad = (angleDeg * Math.PI) / 180;
            const labelDist = R + 18; // Base distance from vertex
            
            let alignClass = "items-center";
            let offsetX = 0;
            let offsetY = 0;

            // Positioning alignments for 5-vertex layout
            if (i === 0) { // Top
            ...
            // Container size for foreignObject (generous bounds)
            const fWidth = 140;
            const fHeight = 48;
            const fX = lx - fWidth / 2;
            const fY = ly - fHeight / 2;
            ...
            return (
              ...
              >
                <div
                  className={cn("flex h-full w-full flex-col justify-center", alignClass)}
                  onMouseEnter={() => handleVertexHover(item, pt)}
                  onMouseLeave={handleVertexLeave}
                >
                  <div
                    className={cn(
                      "flex cursor-pointer flex-col items-center justify-center rounded-xl border  bg-white/70 px-3 py-1.5  backdrop-blur-xl transition-all duration-300 dark: dark:bg-slate-900/80",
                      isHovered ? "scale-105  ring-2 ring-indigo-500/20" : "scale-100"
                    )}
                  >
                    <span
                      className={cn(
                        "text-[9.5px] font-black tracking-widest transition-colors duration-200 uppercase",
                        isHovered
                          ? "text-violet-600 dark:text-violet-400"
                          : "text-[var(--text-primary)]"
                      )}
                    >
                      {isVi ? item.shortLabelVi : item.shortLabelEn}
                    </span>
                    <span
                      className="mt-0.5 text-[11px] font-extrabold leading-none"
        ...
        {/* 8. Interactive Floating Tooltip on Hover for each Skill Vertex */}
        <AnimatePresence>
          {activeTooltip && (
            <motion.div
              ...
              style={{
                ...
                transform: `translate(-50%, ${
                  activeTooltip.rawY < 200 ? "14px" : "-115%"
                })`,
              }}
            >
              {/* Header: Title + Exact Proficiency Badge */}
              <div className="flex items-center justify-between gap-2   pb-2">
                <div className="flex items-center gap-1.5 truncate">
                  <span
                    className="h-2.5 w-2.5 shrink-0 rounded-full"
                    style={{ backgroundColor: activeTooltip.item.color }}
                  />
                  <h4 className="truncate text-xs font-black text-[var(--text-primary)]">
                    {isVi
                      ? activeTooltip.item.titleVi
                      : activeTooltip.item.titleEn}
                  </h4>
                </div>
                ...
                <span
                  ...
                  style={{
                    backgroundColor: `${activeTooltip.item.color}22`,
                    color: activeTooltip.item.color,
                    borderColor: `${activeTooltip.item.color}50`,
                  }}
                >
                  {activeTooltip.item.A}%
                </span>
              </div>

              {/* Description */}
              <p className="mt-2 text-[11px] leading-relaxed font-medium text-[var(--text-secondary)]">
                {isVi ? activeTooltip.item.descVi : activeTooltip.item.descEn}
              </p>
                ...
                <div className="mt-2.5 flex flex-wrap gap-1">
                  {activeTooltip.item.tags.map((tag, tIdx) => (
                    <span
                      ...
                      style={{
                        backgroundColor: `${activeTooltip.item.color}12`,
                        color: activeTooltip.item.color,
                        borderColor: `${activeTooltip.item.color}30`,
                      }}
                    >
                      {tag}
                    </span>
      ...
      {/* Persistent active summary strip below radar when no vertex is hovered */}
      {activeItem && !activeTooltip && (
        <motion.div
          initial={{ opacity: 0, y: 4 }}
          animate={{ opacity: 1, y: 0 }}
          className="mt-2 flex items-center gap-2 rounded-xl border  bg-[var(--card)] px-3.5 py-1.5  backdrop-blur-md"
        >
          <span
          ...
          <span
            className="rounded px-2 py-0.5 text-[10.5px] font-black"
            style={{
...
export function Skills() {
  ...
  // Calculate dynamic average score for each of the 5 skill categories from the live skills array
  ...
  const dynamicRadarData = [
    {
      id: "professional",
      subjectEn: `1. Professional (${professionalAvg}%)`,
      subjectVi: `1. Chuyên môn (${professionalAvg}%)`,
    ...
    {
      id: "innovation",
      subjectEn: `2. Innovation (${innovationAvg}%)`,
      subjectVi: `2. Đổi mới & số hóa (${innovationAvg}%)`,
    ...
    {
      id: "leadership",
      subjectEn: `3. Leadership (${leadershipAvg}%)`,
      subjectVi: `3. Lãnh đạo & quản trị (${leadershipAvg}%)`,
    ...
    {
      id: "collaboration",
      subjectEn: `4. Collaboration (${collaborationAvg}%)`,
      subjectVi: `4. Hợp tác & kết nối (${collaborationAvg}%)`,
    ...
    {
      id: "languages",
      subjectEn: `5. Languages (${languagesAvg}%)`,
      subjectVi: `5. Ngôn ngữ (${languagesAvg}%)`,
  ...
  // Custom renderer for PolarAngleAxis ticks on Radar Chart
  const renderAngleAxisTick = (props: any) => {
    const { x, y, payload, cx, cy } = props;
    if (!payload || !payload.value) return null;

    const item = dynamicRadarData.find(
      (d) =>
        d.shortLabelVi === payload.value ||
        d.shortLabelEn === payload.value ||
        d.subjectVi === payload.value ||
        d.subjectEn === payload.value,
    );

    const score = item ? item.A : "";
    const color = item ? item.color : "#007aff";
    const label = isVi
      ? item?.shortLabelVi || payload.value
      : item?.shortLabelEn || payload.value;
  ...
  return (
    <PageLayout
      ...
      pageId="skills"
      pageName="Skills Main Card"
      title={isVi ? "Kỹ Năng & Năng Lực Cốt Lõi" : "Skills & Core Competencies"}
      subtitle={
        isVi
          ? "Tổng quan kỹ năng chuyên môn, lãnh đạo và vận hành."
          : "Overview of professional, leadership, and operational skills."
      }
      icon={Brain}
      headerActions={
    ...
    >
      <div className="relative mx-auto flex w-full max-w-[1240px] flex-col items-start justify-start gap-6 text-left bg-transparent">
        {/* ====================================================================================== */}
        {/* BIỂU ĐỒ TỐI ƯU THEO PHÂN TÍCH YÊU CẦU NGUYÊN BẢN */}
        {/* ====================================================================================== */}
        <div className="w-full space-y-6 bg-transparent">
          {/* ================================================================================== */}
          {/* KHỐI BIỂU ĐỒ RADAR TỔNG QUAN NĂNG LỰC ĐA CHIỀU (REDESIGNED RADAR CARD) */}
          {/* ================================================================================== */}
          <div className="group magic-card relative w-full space-y-5 overflow-hidden rounded-[15px] border  bg-[var(--card)] p-5 sm:p-6 lg:p-7  backdrop-blur-2xl transition-all duration-300 sub-card with-ripple">
            {/* Top Glow & Decorative Background */}
            ...
            <div className="relative z-10 flex flex-col gap-4   pb-5 lg:flex-row lg:items-center lg:justify-between">
                ...
                <div>
                  <div className="flex items-center gap-2">
                    <h3 className="text-base font-black tracking-tight text-[var(--text-primary)] sm:text-lg">
                      {isVi
                        ? "Biểu Đồ Radar Đa Chiều Năng Lực"
                        : "Multidimensional Skills Radar Chart"}
                    </h3>
                    <span className="hidden rounded-full border  bg-violet-500/50 px-2.5 py-0.5 text-[10px] font-black text-white  backdrop-blur-md sm:inline-block">
                      5 Pillars
                    </span>
                  </div>
                  <p className="mt-0.5 text-xs font-medium text-[var(--muted)]">
                    {isVi
                      ? "Phân tích trực quan mức độ thành thạo và cân bằng giữa 5 trụ cột năng lực"
                      : "Visual analysis of competency balance and mastery across 5 core pillars"}
                  </p>
                </div>
              </div>
              ...
              <div className="flex flex-wrap items-center gap-2 sm:gap-2.5">
                ...
                <div className="flex items-center gap-2 rounded-full border  bg-amber-500/50 px-3.5 py-1.5 text-xs font-black text-white  backdrop-blur-md">
                  <Star size={14} className="text-white" />
                  <span className="text-[11px] font-bold text-white/90">
                    {isVi ? "Thế mạnh:" : "Top Domain:"}
                  </span>
                  <span className="text-xs font-black text-white">
                    {isVi ? topPillar?.shortLabelVi : topPillar?.shortLabelEn} ({topPillar?.A}%)
                  </span>
                </div>
              </div>
            </div>

            {/* 2. Main Content Grid: Left (Interactive Radar Visualizer) + Right (5 Pillar Indicator Cards) */}
            <div className="relative z-10 grid grid-cols-1 items-center gap-6 lg:grid-cols-12">
              {/* LEFT: Guaranteed High-Reliability Interactive SVG Radar Visualizer */}
              ...
              <div className="flex flex-col gap-2.5 lg:col-span-5">
                <div className="flex items-center justify-between px-1 pb-1">
                  <span className="text-[11px] font-black tracking-wider text-[var(--muted)] uppercase">
                    {isVi
                      ? "Chi tiết 5 Trụ cột Năng lực"
                      : "5 Core Competency Pillars"}
                  </span>
                  <span className="text-[10px] font-bold text-[var(--muted)]">
                    {isVi ? "Mức độ chuẩn hóa" : "Standardized"}
                  </span>
                </div>
                  ...
                  return (
                    <div
                      ...
                      className={cn(
                        ...
                        isHovered
                          ? " bg-[var(--surface)]"
                          : " bg-[var(--surface)]/50 hover:bg-[var(--surface)] hover:"
                      )}
                    >
                      ...
                      <div className="flex items-center justify-between gap-2">
                        <div className="flex min-w-0 items-center gap-2.5">
                          ...
                          <div className="min-w-0">
                            <h4 className="truncate text-xs font-extrabold text-[var(--text-primary)]">
                      ...
                      <div className="mt-2 flex flex-wrap items-center gap-1.5">
                        {item.tags?.map((tag, tIdx) => (
                          <span
                            key={tIdx}
                            className="rounded px-1.5 py-0.5 text-[9px] font-bold text-[var(--muted)] bg-[var(--card)] border "
                          >
                            {tag}
                          </span>
                        ))}
            ...
            <div className="relative z-10 mt-1 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 rounded-xl border  bg-gradient-to-r from-violet-500/5 via-[var(--surface)] to-indigo-500/5 p-3.5 text-xs font-medium text-[var(--text-secondary)]">
              <div className="flex items-center gap-2.5">
                <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-violet-500/15 text-violet-600 dark:text-violet-400">
                  <Award size={16} />
                </div>
                <p className="m-0 leading-relaxed text-left text-xs font-semibold text-[var(--text-primary)]">
                  {isVi
                    ? "✨ Đánh giá tổng hợp: Năng lực cân bằng cao giữa kỹ năng công nghệ (CRM/Data/AI) và quản trị lãnh đạo quy mô lớn."
                    : "✨ Summary Assessment: Highly balanced competency across tech/systems (CRM/Data/AI) and large-scale leadership governance."}
                </p>
              </div>
              <div className="flex shrink-0 items-center gap-1.5 text-[11px] font-bold text-violet-600 dark:text-violet-400">
                <ShieldCheck size={14} />
                <span>{isVi ? "Chuẩn hóa 100%" : "100% Standardized"}</span>
              </div>
            </div>
          </div>
      ...
      {/* PILLAR SKILL BLOCKS POPUP MODAL */}
      <AnimatePresence>
        {selectedPillarId && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-950/70 p-4 backdrop-blur-md">
            <motion.div
              ...
              className="relative w-full overflow-y-auto bg-[var(--bg)] p-2 sm:p-6 popup-card-80 custom-scrollbar"
            >
               <button
                onClick={() => {
                  playUiSound("click");
                ...
                className="absolute top-4 right-4 z-50 cursor-pointer rounded-full bg-rose-500/10 text-rose-500 p-2 transition-all hover:bg-rose-500 hover:text-white"
               >
                 <X size={20} />
               </button>

               <div className="mt-8 relative z-10 w-full mx-auto max-w-4xl">
                 {selectedPillarId === "professional" && (
                    <>
                    {/* KHỐI 1: KỸ NĂNG CHUYÊN MÔN */}
            <div className="group magic-card relative space-y-5 overflow-hidden rounded-[15px] border  bg-[var(--card)] p-6  backdrop-blur-2xl sub-card with-ripple">
              {/* Large Category Watermark Background Icon */}
              <div className="pointer-events-none absolute -right-6 -bottom-6 z-0 text-[#007aff] opacity-[0.08] transition-opacity duration-500 group-hover:opacity-[0.20] dark:opacity-[0.14]">
                <Database size={180} />
              </div>

              <div className="relative z-10 flex flex-col items-start gap-3   pb-4 text-left">
                <div className="flex items-center gap-3">
                  <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full border  bg-[#007aff]/50 text-white  backdrop-blur-md">
                    <Database size={18} className="shrink-0 text-white" />
                  </div>
                  <h3 className="text-base font-black tracking-wide text-[#007aff] sm:text-lg">
                    1. {isVi ? "Kỹ năng chuyên môn" : "1. Professional skills"}
                  </h3>
                </div>
                <p className="pl-12 text-xs font-medium text-[var(--muted)] text-left">
                  {isVi
                    ? "Chi tiết các bộ kỹ năng chuyên môn, công cụ CRM/BI & quy trình SOPs"
                    : "Detailed professional skill sets, CRM/BI tools & SOPs"}
                </p>
              </div>

              {/* PROGRESS METRICS LIST (ADJUSTED TO 1 COLUMN VIEW) */}
              <div className="relative z-10 grid grid-cols-1 gap-3.5">
                {contentData.skills.professional.map((item, idx) => (
                  <SkillFlipCard
                    key={idx}
                    skill={item}
                    color="#007aff"
                 ...
                 {selectedPillarId === "innovation" && (
                    <>
                    {/* KHỐI 2: KỸ NĂNG ĐỔI MỚI & SỐ HÓA */}
            <div className="group magic-card relative space-y-5 overflow-hidden rounded-[15px] border  bg-[var(--card)] p-6  backdrop-blur-2xl sub-card with-ripple">
              {/* Large Category Watermark Background Icon */}
              <div className="pointer-events-none absolute -right-6 -bottom-6 z-0 text-[#ff9500] opacity-[0.08] transition-opacity duration-500 group-hover:opacity-[0.20] dark:opacity-[0.14]">
                <Sparkles size={180} />
              </div>

              <div className="relative z-10 flex flex-col items-start gap-3   pb-4 text-left">
                <div className="flex items-center gap-3">
                  <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full border  bg-[#ff9500]/50 text-white  backdrop-blur-md">
                    <Sparkles size={18} className="shrink-0 text-white" />
                  </div>
                  <h3 className="text-base font-black tracking-wide text-[#ff9500] sm:text-lg">
                    2.{" "}
                    {isVi
                      ? "Kỹ năng đổi mới & số hóa"
                      : "2. Innovation & Digital Transformation"}
                  </h3>
                </div>
                <p className="pl-12 text-xs font-medium text-[var(--muted)] text-left">
                  {isVi
                    ? "Tự động hóa quy trình (AI Chatbot, Zapier/n8n), Kaizen & quản trị BCP"
                    : "Process automation (AI, Zapier/n8n), Kaizen & BCP governance"}
                </p>
              </div>

              {/* INNOVATION FLIP CARDS GRID */}
              <div className="relative z-10 grid grid-cols-1 gap-3.5">
                {contentData.skills.innovation.map((item, idx) => (
                  <SkillFlipCard
                    key={idx}
                    skill={item}
                    color="#ff9500"
                 ...
                 {selectedPillarId === "leadership" && (
                    <>
                    {/* KHỐI 3: KỸ NĂNG LÃNH ĐẠO */}
            <div className="group magic-card relative space-y-5 overflow-hidden rounded-[15px] border  bg-[var(--card)] p-6  backdrop-blur-2xl sub-card with-ripple">
              {/* Large Category Watermark Background Icon */}
              <div className="pointer-events-none absolute -right-6 -bottom-6 z-0 text-[#ff3b30] opacity-[0.08] transition-opacity duration-500 group-hover:opacity-[0.20] dark:opacity-[0.14]">
                <Users2 size={180} />
              </div>

              <div className="relative z-10 flex flex-col items-start gap-3   pb-4 text-left">
                <div className="flex items-center gap-3">
                  <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full border  bg-[#ff3b30]/50 text-white  backdrop-blur-md">
                    <Users2 size={18} className="shrink-0 text-white" />
                  </div>
                  <h3 className="text-base font-black tracking-wide text-[#ff3b30] sm:text-lg">
                    3.{" "}
                    {isVi
                ...
                <p className="pl-12 text-xs font-medium text-[var(--muted)] text-left">
                  {isVi
                    ? "Biểu đồ năng lực quản trị quy mô kết hợp các Trụ cột Chứng minh Thực tế"
                    : "Management competency + Leadership Achievement Pillars"}
                </p>
              </div>
              ...
              <div className="relative z-10 grid grid-cols-1 gap-4">
                {[
                  {
                    title: isVi ? "Lãnh đạo Nhân sự" : "Staff Leadership",
                    desc: isVi
                      ? "Xây dựng bộ máy VED (Garena) & MoMo mượt mà"
                      : "Built scalable CS & Contact Center teams",
                    icon: Users2,
                    color: "#ff3b30",
                    score: "92%",
                  },
                  {
                    title: isVi
                      ? "Quản trị KPI & CSAT > 95%"
                      : "KPI & CSAT > 95%",
                    desc: isVi
                      ? "Cam kết SLA > 98%, nâng tầm dịch vụ"
                      : "SLA commitments and quality standards",
                    icon: Gauge,
                    color: "#30d158",
                    score: "95%",
                  },
                  {
                    title: isVi
                      ? "Ứng phó Khủng hoảng BCP"
                      : "BCP Crisis Management",
                    desc: isVi
                      ? "Đảm bảo tổng đài vận hành 24/7 thông suốt"
                      : "Ensured 24/7 uninterrupted operations",
                    icon: ShieldCheck,
                    color: "#007aff",
                    score: "88%",
                  },
                  {
                    title: isVi
                      ...
                      : "Training & Mentorship",
                    desc: isVi
                      ? "Đào tạo hàng nghìn tư vấn viên chuẩn nghề"
                      : "Trained 1000s of professional agents",
                    icon: GraduationCap,
                    color: "#ff9500",
                    score: "90%",
                  },
                  ...
                  return (
                    <div
                      key={idx}
                      className="group magic-card relative space-y-2 overflow-hidden rounded-xl border  bg-[var(--card)] p-4 text-left transition-all hover:"
                    >
                      <div className="flex items-center justify-between">
                        ...
                        <span className="rounded bg-[var(--bg)] px-2 py-0.5 text-xs font-black text-[var(--text-primary)]">
                          {pillar.score}
                        </span>
                      </div>
                      <p className="text-xs leading-snug font-bold text-[var(--text-secondary)]">
                        {pillar.desc}
                      </p>
                 ...
                 {selectedPillarId === "collaboration" && (
                    <>
                    {/* KHỐI 4: KỸ NĂNG HỢP TÁC & KẾT NỐI */}
            <div className="magic-card group relative space-y-5 overflow-hidden rounded-[15px] border  bg-[var(--card)] p-6  backdrop-blur-2xl sub-card with-ripple">
              {/* Large Category Watermark Background Icon */}
              <div className="pointer-events-none absolute -right-6 -bottom-6 z-0 text-[#af52de] opacity-[0.08] transition-opacity duration-500 group-hover:opacity-[0.20] dark:opacity-[0.14]">
                <Network size={180} />
              </div>

              <div className="relative z-10 flex flex-col items-start gap-3   pb-4 text-left">
                <div className="flex items-center gap-3">
                  <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full border  bg-[#af52de]/50 text-white  backdrop-blur-md">
                    <Network size={18} className="shrink-0 text-white" />
                  </div>
                  <h3 className="text-base font-black tracking-wide text-[#af52de] sm:text-lg">
                    4.{" "}
                    {isVi
                      ? "Kỹ năng hợp tác & kết nối"
                      : "4. Cross-functional collaboration"}
                  </h3>
                </div>
                <p className="pl-12 text-xs font-medium text-[var(--muted)] text-left">
                  {isVi
                    ? "Sơ đồ mạng lưới tương tác đa chiều giữa CSKH với các khối Technology & Marketing"
                    : "Network nodes between CS, IT, Product, Legal and Marketing"}
                </p>
              </div>
              ...
              <div className="relative z-10 grid grid-cols-1 gap-4">
                {[
                  {
                    department: "IT & Software Dev",
                    impact: isVi
                      ? "Phối hợp phản hồi & sửa lỗi sản phẩm nhanh"
                      : "Bug fixing & feature integration",
                    score: "90%",
                    color: "#007aff",
                    icon: Code,
                  },
                  {
                    ...
                    impact: isVi
                      ? "Truyền tải VOC (Ý kiến KH) để cải tiến UX"
                      : "Voice of Customer UX enhancements",
                    score: "92%",
                    color: "#30d158",
                    icon: Layers,
                  },
                  {
                    ...
                    impact: isVi
                      ? "Chuẩn hóa quy trình tuân thủ & bảo mật"
                      : "Compliance SOPs & security standards",
                    score: "88%",
                    color: "#ff9500",
                    icon: ShieldCheck,
                  },
                  {
                    ...
                    impact: isVi
                      ? "Đồng bộ chương trình khuyến mãi & truyền thông"
                      : "Campaign alignment & reputation protection",
                    score: "89%",
                    color: "#af52de",
                    icon: Presentation,
                  },
                 ...
                 {selectedPillarId === "languages" && (
                    <>
                    {/* ================================================================================== */}
          {/* KHỐI 5: KỸ NĂNG NGÔN NGỮ */}
          {/* ================================================================================== */}
          <div className="magic-card relative z-10 space-y-5 rounded-[15px] border  bg-[var(--card)] p-6  backdrop-blur-2xl sub-card with-ripple">
            <div className="flex flex-col items-start gap-3   pb-4 text-left">
              <div className="flex items-center gap-3">
                <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full border  bg-[#34c759]/50 text-white  backdrop-blur-md">
                  <Globe size={18} className="shrink-0 text-white" />
                </div>
                <h3 className="text-base font-black tracking-wide text-[var(--text-primary)] sm:text-lg">
                  5. {isVi ? "Kỹ năng ngôn ngữ" : "5. Language proficiency"}
                </h3>
              </div>
              <p className="pl-12 text-xs font-medium text-[var(--muted)] text-left">
                {isVi
                  ? "Biểu đồ trình độ theo chuẩn quốc tế (Nghe, Nói, Đọc, Viết) trong môi trường chuyên nghiệp"
                  : "Circular Dial Rings for Native Vietnamese & Professional Working English"}
              </p>
            </div>

            <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
              {/* TIẾNG VIỆT */}
              <div className="flex items-center justify-between gap-4 rounded-xl border  bg-gradient-to-br from-[#34c759]/10 via-[var(--card)] to-[#34c759]/5 p-5  transition-colors">
                <div className="space-y-2 text-left">
                  ...
                  <p className="text-xs font-medium text-[var(--muted)]">
                    {isVi
                      ? "Ngôn ngữ mẹ đẻ, thuyết trình, soạn thảo văn bản pháp lý & báo cáo cấp cao"
                      : "Native fluency for executive presentations and formal reports"}
                  </p>
                  <div className="flex flex-wrap gap-1.5 pt-1">
                    {["Thuyết trình", "Soạn SOPs", "Đào tạo", "Đàm phán"].map(
                      (t, idx) => (
                        <span
                          key={idx}
                ...
                <div className="flex h-20 w-20 shrink-0 flex-col items-center justify-center rounded-full   text-center ">
              ...
              {/* TIẾNG ANH */}
              <div className="flex items-center justify-between gap-4 rounded-xl border  bg-gradient-to-br from-[#007aff]/10 via-[var(--card)] to-[#007aff]/5 p-5  transition-colors">
                <div className="space-y-2 text-left">
                  <div className="flex items-center gap-2">
                    <span className="h-3 w-3 rounded-full bg-[#007aff]" />
                    <h4 className="text-base font-black text-[var(--text-primary)]">
                      {isVi
                        ? "Tiếng Anh (Working Professional)"
                        : "English (Working Proficiency)"}
                    </h4>
                  </div>
                  <p className="text-xs font-medium text-[var(--muted)]">
                    {isVi
                      ? "Giao tiếp công việc, đọc tài liệu chuyên ngành CRM/AI và trao đổi với đối tác"
                      : "Business communication, reading technical CRM & AI docs"}
                  </p>
                  <div className="flex flex-wrap gap-1.5 pt-1">
                    {[
                      "Reading Docs",
                      ...
                      <span
                        key={idx}
                        className="rounded bg-blue-500/10 px-2 py-0.5 text-[10px] font-bold text-blue-600 dark:text-blue-400"
                      >
                        {t}
                      </span>
                    ))}
      ...
      {/* SKILL DETAIL MODAL (MODAL CHI TIẾT KHI CLICK VÀO KỸ NĂNG) */}
        ...
        {activeSkillModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/70 p-4 backdrop-blur-md">
            <motion.div
              ...
              exit={{ opacity: 0, scale: 0.95, y: 15 }}
              transition={{ duration: 0.2 }}
              className="magic-card relative w-[80vw] max-w-[80vw] space-y-4 rounded-[15px]   bg-[var(--card)] p-6 text-left  backdrop-blur-2xl sub-card with-ripple"
            >
              <button
                onClick={() => {
                  playUiSound("click");
                ...
                className="absolute top-4 right-4 cursor-pointer rounded-full bg-[var(--bg)] p-2 transition-all hover:bg-rose-500 hover:text-white"
              >
                <X size={16} />
              </button>

              <div className="flex items-center gap-3 pr-8">
                <div className="rounded-xl border  bg-purple-500/15 p-3 text-[#5e5ce6] dark:text-[#5e5ce6]">
                  <Brain size={24} />
                </div>
                <div>
                  ...
                  <span className="rounded-full border  bg-purple-500/10 px-2.5 py-0.5 text-xs font-black text-[#5e5ce6] dark:text-[#5e5ce6]">
                    {isVi
                      ? `Mức độ thuần thục: ${activeSkillModal.level}%`
                      : `Mastery level: ${activeSkillModal.level}%`}
                  </span>
                </div>
              </div>
              ...
              {SKILL_DETAILS_MAP[activeSkillModal.name] ? (
                <div className="space-y-3.5 pt-2 text-xs">
                  <div className="space-y-1 rounded-xl border  dark: bg-purple-50/70 dark:bg-purple-950/10 p-3">
                    <span className="text-[10px] font-extrabold text-purple-700 dark:text-purple-300 uppercase">
                      {isVi ? "Mô tả năng lực:" : "Capability description:"}
                    </span>
                    <p className="m-0 text-justify text-[11.5px] leading-relaxed font-semibold text-slate-800 dark:text-slate-200">
                      {SKILL_DETAILS_MAP[activeSkillModal.name].desc}
                    </p>
                  </div>
                  ...
                  <div className="space-y-1 rounded-xl border  dark: bg-indigo-50/70 dark:bg-indigo-950/10 p-3">
                    <span className="text-[10px] font-extrabold text-indigo-700 dark:text-indigo-300 uppercase">
                      {isVi
                        ...
                        : "Practical cases & projects:"}
                    </span>
                    <p className="m-0 text-justify text-[11.5px] leading-relaxed font-semibold text-slate-800 dark:text-slate-200">
                      {SKILL_DETAILS_MAP[activeSkillModal.name].application}
                    </p>
                  </div>
                  ...
                  <div className="space-y-1.5">
                    <span className="text-[10px] font-bold text-[var(--muted)] uppercase">
                      {isVi
                        ? "Công cụ & khung chuẩn:"
                        : "Tools & framework standards:"}
                    </span>
                    <div className="flex flex-wrap gap-1.5">
                      {SKILL_DETAILS_MAP[activeSkillModal.name].tools.map(
                        (t, idx) => (
                          <span
                            key={idx}
                            className="rounded-lg border  bg-[#5e5ce6]/10 px-2.5 py-1 text-[11px] font-black text-[#5e5ce6]"
                          >
                            {t}
                          </span>
                        ),
                ...
                <div className="rounded-xl bg-[var(--bg)] p-4 text-justify text-xs leading-relaxed font-medium text-[var(--muted)]">
                  {isVi
                    ? "Kỹ năng chuyên môn được tích lũy qua hơn 22 năm quản trị vận hành Chăm sóc Khách hàng tại Mobifone, VED (Shopee/Garena), Prudential, MoMo và Finviet."
                    : "Specialized skill acquired through over 22 years of executive Customer Experience operations at Mobifone, VED (Shopee/Garena), Prudential, MoMo, and Finviet."}
                </div>
              )}
              ...
              <div className="flex justify-end   pt-3">
                <button
                  ...
                  className="cursor-pointer rounded-xl bg-[#5e5ce6] px-4 py-2 text-xs font-black text-white  transition-all hover:bg-[#5e5ce6]/90"
                >
