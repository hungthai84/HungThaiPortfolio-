...
export function PrinciplesSection() {
  ...
  return (
    <div
      className="rounded-none border border-[var(--border)] bg-[var(--card)]/90 dark:bg-slate-900/90 backdrop-blur-xl p-5 sm:p-6 shadow-lg flex flex-col justify-between h-full"
      id="principles-section"
    >
      <div className="flex items-center gap-2 mb-3">
        <span className="h-2.5 w-2.5 rounded-none bg-indigo-600 dark:bg-indigo-400" />
        ...
        {PILLARS_DATA.map((item) => {
          ...
          return (
            <div
              key={item.id}
              className={cn(
                "rounded-none border p-3 sm:p-4 flex flex-col items-center justify-center text-center shadow-2xs transition-all duration-300 hover:shadow-md hover:-translate-y-0.5",
                item.gradientClass,
                item.borderClass
              )}
            >
              <div
                className={cn(
                  "w-8 h-8 sm:w-9 sm:h-9 rounded-none flex items-center justify-center mb-2 shadow-xs",
                  item.iconBoxClass
                )}
              >
                <Icon size={16} />
...
export function CoreValuesSection() {
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 mb-3">
        <span className="h-2.5 w-2.5 rounded-none bg-blue-600 dark:bg-sky-400" />
        <h4 className="text-xs sm:text-[13px] font-bold text-slate-800 dark:text-slate-200">
          {isVi
