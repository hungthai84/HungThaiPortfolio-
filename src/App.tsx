...
export default function App() {
  ...
  useEffect(() => {
    const handleVp = (e: Event) => {
      ...
      if (custom.detail?.mode) {
        ...
        localStorage.setItem("app_viewport_mode", custom.detail.mode);
      }
    };
  ...
  return (
    ...
    >
      ...
      {/* Top Floating Responsive Preview Device Bar when in simulated mode */}
      {viewportMode !== "desktop" && (
        <div className="fixed top-4 left-1/2 -translate-x-1/2 z-[9999] flex items-center gap-3 px-4 py-2 rounded-2xl bg-slate-900/90 border border-slate-700/80 text-white shadow-2xl backdrop-blur-xl animate-in fade-in slide-in-from-top-4 duration-300">
          <div className="flex items-center gap-2 pr-2 border-r border-slate-700">
            ...
            <span className="text-xs font-black text-slate-200">
              {viewportMode === "tablet"
                ...
                ? "📱 Mobile Compact (iPhone SE • 375px)"
                : "💻 Small Laptop (1024px)"}
            </span>
          </div>

          {/* Quick Viewport Switchers */}
          <div className="flex items-center gap-1">
            {[
              { id: "desktop", label: "Desktop" },
              { id: "laptop", label: "1024px" },
              { id: "tablet", label: "768px" },
              { id: "mobile", label: "390px" },
              { id: "mobile-sm", label: "375px" },
            ].map((vp) => (
              <button
                key={vp.id}
                type="button"
                onClick={() => {
                  playUiSound("click");
                  setViewportMode(vp.id);
                  localStorage.setItem("app_viewport_mode", vp.id);
                  window.dispatchEvent(
                    new CustomEvent("app-set-viewport-mode", { detail: { mode: vp.id } })
                  );
                }}
                className={cn(
          ...
          <button
            ...
            className="ml-1 p-1 rounded-lg text-slate-400 hover:text-white hover:bg-slate-800 transition-colors cursor-pointer text-xs font-bold"
            title="Thoát chế độ mô phỏng / Reset về Full Desktop"
          >
            ✕
          </button>
        </div>
      ...
      <div
        className={cn(
          ...
          viewportMode === "desktop"
            ...
            : viewportMode === "tablet"
            ? "w-full max-w-[768px] mx-auto min-h-screen pt-16 pb-8"
            : viewportMode === "mobile"
            ? "w-full max-w-[390px] mx-auto min-h-screen pt-16 pb-8"
            : viewportMode === "mobile-sm"
            ? "w-full max-w-[375px] mx-auto min-h-screen pt-16 pb-8"
            : "w-full max-w-[1024px] mx-auto min-h-screen pt-16 pb-8",
        )}
      >
        ...
        <section
          ...
          className={cn(
            ...
            viewportMode !== "desktop" &&
              "ring-8 ring-slate-800/80 dark:ring-slate-700/80 rounded-[36px] shadow-[0_25px_60px_-15px_rgba(0,0,0,0.6)]",
